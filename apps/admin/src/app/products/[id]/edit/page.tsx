...
export default function AdminEditProductPage({
  ...
  const [name, setName] = useState('Gold Pleksi "Happy Birthday" Pasta Süsü');
  const [sku, setSku] = useState('GB-TOPPER-GLD-01');
  const [category, setCategory] = useState('Pasta Süsleri & Topperlar');
  const [price, setPrice] = useState('49.90');
  const [discountPrice, setDiscountPrice] = useState('39.90');
  const [costPrice, setCostPrice] = useState('15.00');
  const [stock, setStock] = useState('120');
  const [status, setStatus] = useState<'ACTIVE' | 'DRAFT'>('ACTIVE');
  const [description, setDescription] = useState(
    'Yüksek kaliteli aynalı akrilik pleksi malzemeden üretilmiştir. Doğum günü pastaları, nişan ve özel kutlama tasarımlarına eşsiz bir şıklık ve parıltı katar.'
  );
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = (event) => {
      ...
      if (result) {
        setImageUrl(result);
        setNotification('Görsel başarıyla seçildi ve güncellendi!');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setNotification(`"${name}" ürünü başarıyla güncellendi!`);
    setTimeout(() => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <h1 className="text-2xl font-extrabold text-slate-900 mt-0.5">Ürünü Düzenle: {name}</h1>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
            ...
            <div className="space-y-4 text-xs">
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">Ürün Açıklaması</label>
                <textarea
                  rows={5}
                  value={description}
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
            ...
            <div className="space-y-4 text-xs">
              ...
              {imageUrl && (
                <div className="pt-2">
                  <label className="font-bold text-slate-700 block mb-2">Mevcut / Yüklenen Görsel Önizlemesi</label>
                  <div className="flex items-center space-x-4 p-3 bg-slate-50 rounded-2xl border border-slate-200">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-slate-800 text-xs truncate">Ürün Görseli Aktif</p>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">Maliyet (TL)</label>
                <input
                  type="number"
                  step="0.01"
                  value={costPrice}
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
            ...
            <div className="space-y-4 text-xs">
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">Stok Miktarı</label>
                <input
                  type="number"
                  value={stock}
